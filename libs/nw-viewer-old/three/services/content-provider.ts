...
export class ContentProvider implements GameService {
  ...
  public modelSource(url: string, material: string, rootUrl: string): ModelSource | null {
    ...
    params.set('yup', '')
    params.set('no-proxy', '') // skips shadow proxy nodes
    params.set('no-lod', '') // skips lod nodes
    return {
...
function searchParams(url: string) {
  return new URL(url, 'http://example.com').searchParams
}

function withSearchParams(url: string, params: URLSearchParams) {
  url = url.split('?')[0]
  if (params.toString()) {
    url += '?' + params.toString()
  }
